...
class IsEventAuthorOrModeratorOrReadOnly(permissions.BasePermission):
    """
    Allow edit/delete/manage only if:
    - User is a Platform Admin or Platform Moderator.
    - User is the author of the event.
    - If the event is community-scoped: User is an Owner, Admin, or Moderator in that community.
    - If the event is college-scoped: User is a Moderator or Admin and belongs to that college.
    """
    ...
    def has_read_permission(self, request, obj) -> bool:
        ...
        # Platform Admin/Mod can read anything
    ...
    def has_permission(self, request, view) -> bool:
        ...
        # Create action checks role and scope
    ...
    def has_object_permission(self, request, view, obj) -> bool:
        ...
        social_actions = ["join", "leave", "check_in", "remind", "comments", "ical"]
        if request.method in permissions.SAFE_METHODS or view.action in social_actions:
        ...
        # Write actions: Edit, Update, Delete, Gallery upload, Speaker/Sponsor management
...
class IsEventCommentAuthorOrModeratorOrReadOnly(permissions.BasePermission):
    """
    Allows comments access:
    - Edit/Delete comment: comment author, event author, platform admin/mod, community owner/admin/mod (if community-scoped).
    """
    ...
    def has_object_permission(self, request, view, obj) -> bool:
        ...
        if obj.event.scope == "community" and obj.event.community:
            return obj.event.community.memberships.filter(
                user=request.user,
                role__in=[
                    CommunityMember.Role.OWNER,
                    CommunityMember.Role.ADMIN,
